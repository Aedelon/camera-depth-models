...
if [ ! -f "pyproject.toml" ] || [ ! -d "rgbddepth" ]; then
    echo "NO Erreur: Exécute ce script depuis le répertoire camera-depth-models"
    exit 1
fi

# Vérifier que Git n'est pas déjà initialisé
if [ -d ".git" ]; then
    echo "WARN  Git déjà initialisé dans ce répertoire"
...
git branch -M main
echo "OK Branche renommée en main"
...
if git remote | grep -q "origin"; then
    echo "WARN  Remote 'origin' existe déjà"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo "OK Remote mis à jour avec SSH"
    fi
else
    git remote add origin "$SSH_URL"
...
if ssh -T git@github.com 2>&1 | grep -q "successfully authenticated"; then
    echo "OK Authentification SSH réussie"
else
    echo "WARN  Impossible de se connecter à GitHub via SSH"
    echo ""
...
echo "   • Public OK"
echo "   • NO NE PAS initialiser avec README (déjà présent)"
echo "   • NO NE PAS ajouter .gitignore (déjà présent)"
echo "   • NO NE PAS ajouter license (déjà présent)"
echo ""
...
echo "OK Tag v1.0.2 créé et pushé"

echo ""
echo "=========================================="
echo "OK Setup complet !"
echo "=========================================="
echo ""
...
echo "     → Settings → Features → Discussions OK"
